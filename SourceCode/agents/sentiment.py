...
def sentiment_analyst_agent(state: AgentState):
    ...
    if not end_date or not tickers:
        progress.update_status("sentiment_analyst_agent", None, "Failed: Missing end_date or tickers in state.")
        return state 
    ...
    data_update = {
        "analyst_signals": {
            "sentiment_analyst_agent": sentiment_analysis
        }
    }
