...
def stanley_druckenmiller_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        def normalize(analysis_result):
            if analysis_result.get('max_score', 0) > 0:
                return (analysis_result['score'] / analysis_result['max_score']) * 10
            return 0
        
        growth_norm = normalize(growth_momentum_analysis)
        risk_norm = normalize(risk_reward_analysis)
        valuation_norm = normalize(valuation_analysis)
        sentiment_norm = normalize(sentiment_analysis)
        insider_norm = normalize(insider_activity)
    ...
    data_update = {
        "analyst_signals": {
            "stanley_druckenmiller_agent": druck_analysis
        }
    }
...
def analyze_growth_and_momentum(financial_line_items: list, prices: list) -> dict:
    ...
    return {"score": raw_score, "max_score": max_raw_score, "details": "; ".join(details)}
