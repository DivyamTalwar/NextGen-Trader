...
class Backtester:
    ...
    def execute_trade(self, ticker: str, action: str, quantity: float, current_price: float):
        ...
        if action == "buy":
            ...
            if cost <= self.portfolio["total_cash"]:
                ...
                self.portfolio["total_cash"] -= cost
                return quantity
            else:
                # Calculate maximum affordable quantity
                max_quantity = int(self.portfolio["total_cash"] / current_price)
                if max_quantity > 0:
            ...
            if quantity > 0:
                ...
                position["long"] -= quantity
                self.portfolio["total_cash"] += quantity * current_price
            ...
            if margin_required <= self.portfolio["total_cash"]:
                ...
                self.portfolio["total_cash"] += proceeds
                self.portfolio["total_cash"] -= margin_required
                return quantity
            else:
                # Calculate maximum shortable quantity
                margin_ratio = self.portfolio["margin_requirement"]
                if margin_ratio > 0:
                    max_quantity = int(self.portfolio["total_cash"] / (current_price * margin_ratio))
                else:
                    max_quantity = 0
            ...
            if quantity > 0:
                ...
                self.portfolio["total_cash"] += margin_to_release
                self.portfolio["total_cash"] -= cover_cost
    ...
    def calculate_portfolio_value(self, current_prices):
        """
        Calculate Net Liquidation Value (NLV) of the portfolio.
        NLV = Cash + Market Value of Longs - Market Value of Shorts.
        The 'margin_used' is cash held by the broker, so it's part of our equity.
        """
        # Start with cash that is truly free, plus the cash held as margin
        total_equity_in_cash = self.portfolio["total_cash"] + self.portfolio.get("margin_used", 0)
        
        market_value_of_longs = 0
        market_value_of_shorts = 0

        for ticker, position in self.portfolio["positions"].items():
            price = current_prices.get(ticker)
            if price is not None:
                market_value_of_longs += position["long"] * price
                market_value_of_shorts += position["short"] * price

        total_value = total_equity_in_cash + market_value_of_longs - market_value_of_shorts
        return total_value
        ...
        for current_date in dates:
            ...
            date_rows.append(
                format_backtest_row(
                    ...
                    cash_balance=self.portfolio["total_cash"],
                    total_position_value=total_value - self.portfolio["total_cash"],
